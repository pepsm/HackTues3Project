...
void selected_color(int, CvScalar *, int *);
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    if (dM10 / dArea >= 0 && dM10 / dArea < 1280 && dM01 / dArea >= 0 &&
        dM01 / dArea < 1280 /* to control the limit */) {
    ...
    if (dArea > 10000) {
      ...
      if (magnitude > 0 && magnitude < 100 && current_x > 120 &&
          current_x < 530) {
        // Draw a line from the previous point to the current point
        line(imgLines, cvPoint(current_x, current_y), cvPoint(last_x, last_y),
             lineColor, size_pointer, CV_AA);
      }
    ...
    int key = waitKey(30);
    if (key == 27)  // wait for 'esc' key press for 30ms. If 'esc' key is
                    // pressed, break loop
    {
      ...
      compression_params.push_back(
          CV_IMWRITE_JPEG_QUALITY);  // specify the compression technique

      compression_params.push_back(98);  // specify the compression quality
      /* initialize random seed: */

      bool bSuccess = imwrite(("C:/Users/PC/Desktop/Image.jpg"), imgOriginal,
      ...
      CvScalar *color_p = &lineColor;
      int *size_p = &size_pointer;
      selected_color(key, color_p, size_p);
    }
  }
...
void selected_color(int input_key, CvScalar *lineColor, int *size_pointer) {
  switch (input_key) {
    case 98:
      *lineColor = blue;
      *size_pointer = 6;
...
void display_controls() {
  namedWindow("Control", 9000);  // create a window called "Control"
  moveWindow("Control", 5, 475);
